...
const {
	...
	getPubliserFromBook,
	insertNewPublisher,
	getPubliserWithBookIDFromBook,
	getPubliserWithPublisherIDFromPublisher,
...
(async () => {
	...
	if (dbArray.length == 1) {
		//check if dbArray has a record.  if true drop current VAIN_db and create a new on
		await pgtools.dropdb(credentialsMain, 'vain'); //drop current vain_db
		await pgtools.createdb(credentialsMain, 'vain'); //create new vain_db
	} else {
		//if not true just create vain_db without drop
		await pgtools.createdb(credentialsMain, 'vain'); // create new vain_db
	}

	await executeCommand(createDB); //execute sql to create tables

	await Promise.all(
		bookArray.map(async (elem) => {
			//loop through bookArray[] and insert into book table
			try {
				//pass sql and vars to insertObject()
				await insertObject(insertBook, [
					elem.type,
					elem.hadHelp,
					elem.genre,
					elem.title,
	...
	await Promise.all(
		publisherArray.map(async (elem) => {
			//loop through publisherArray[] and insert into publisher table
			try {
	...
	await getObject(
		getPubliserWithPublisherIDFromPublisher,
		publisherPublisherArray
	); // fill publisherPublisherArray[] with plublisher info from publisher table

	await matchPublisherBook(); // match publisher data to book data and push it to publisheridBookidArray[]
	await Promise.all(
		publisheridBookidArray.map(async (elem) => {
			// loop through publisheridBookidArray[] and insert it into publisher_book table
			try {
				//pass sql and vars to insertObject()
				await insertObject(insertNewPublisherBook, [
					elem.publisherid,
					elem.bookid,
				]);
			} catch (error) {
	...
	await Promise.all(
		authorArray.map(async (elem) => {
			// loop through authorArray[] and insert it into author table
			try {
				//pass sql and vars to insertObject()
	...
	await Promise.all(
		authoridBookidArray.map(async (elem) => {
			// loop through authoridBookidArray[] and insert it into author_book table
			try {
	...
	await Promise.all(
		typeBookArray.map(async (elem) => {
			//loop through typBookArray[] and insert into type_book table
			try {
	...
	await Promise.all(
		subjectBookArray.map(async (elem) => {
			//loop thgrough subjectBookArray[] and insert subject and book id into subject_book table
			try {
